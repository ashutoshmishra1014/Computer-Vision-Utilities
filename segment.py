...
# from lib.modules import batchnormsync
...
def train(train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # save_input_target_images(input,"/home/ashutosh/Documents/Playground/_ve_/mg_projects/handwriting_challenge/code/drn/input", "input")
        # save_input_target_images(target, "/home/ashutosh/Documents/Playground/_ve_/mg_projects/handwriting_challenge/code/drn/target", "target")
...
def train_seg(args):
    ...
    if args.evaluate:
        ...
        # validate(val_loader, model, criterion, eval_score=calc_IOU)
        return
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # train(train_loader, model, criterion, optimizer, epoch, eval_score=calc_IOU)

        # evaluate on validation set
        prec1 = validate(val_loader, model, criterion, eval_score=accuracy)

        # prec1 = validate(val_loader, model, criterion, eval_score=calc_IOU)
...
def save_input_target_images(predictions, output_dir, type):
    global count_input
    global count_target
    print(type)
    print(predictions.shape)
    for index in range(predictions.shape[0]):
        image = predictions[index]
        if(type=="input"):
            image = np.transpose(image, (1, 2, 0))
            # image[:, :, 0].fill_(0)
            # print(image[:,:,0])
        if(type=="input"):
            scipy.misc.imsave(output_dir+"/input-"+str(count_input)+".jpg", image)
            count_input = count_input + 1
            print("input done")
        elif(type=="target"):
            scipy.misc.imsave(output_dir+"/target-"+str(count_target)+".jpg", image)
...
def parse_args():
    ...
    if args.bn_sync:
        print("BatchNormSyncing started.............")
        drn.BatchNorm = batchnormsync.BatchNormSync
        print("BatchNormSyncing ended.............")
